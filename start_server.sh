...
if [ -f /.dockerenv ]; then
    ...
    tmux display-message "Running inside Docker, starting server without reload."
    uvicorn app.server:app --host 0.0.0.0 --port "$SERVER_PORT" --log-config logging_prod.ini &
    UVICORN_PID=$!

    tmux display-message "Starting Celery worker"
    celery -A app.core.celery_app worker -B --autoscale=16,4 &
    CELERY_WORKER_PID=$!

    tmux display-message "Starting Flower on port $FLOWER_PORT"
